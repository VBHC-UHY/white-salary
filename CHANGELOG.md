...
## [0.1.12] - 2026-07-26 · 安全修复与"静默失效"专项

这个修订版是一次专项排查的结果，主题是**不再让白无声地坏掉**。修的问题有一个共同形状：
调用方按一套约定去用，被调方是另一套，中间隔着一个吞掉一切的 `except`，
于是功能整体失效却零报错——日志里干净，测试也是绿的。

⚠️ **含一个安全修复，建议尽快升级**：如果你安装过第三方插件，请优先更新。

### 安全

- **插件风险筛查存在提权缺口（重要）**：插件导入检查此前是黑名单语义（未明确禁止即放行），
  而 `os` / `sys` 不在黑名单也不在任何权限桶里，等于完全放行。插件**不声明任何权限**即可
  读取 `os.environ` 里的全部 API key、用 `os.remove` 删除项目文件、
  用 `run = os.popen` 先取值再调用绕过按名字的进程黑名单执行任意命令、
  以及篡改 `sys.modules` 劫持宿主逻辑。
  - 导入检查翻转为白名单：未知模块需在 `config.json` 的 `permissions` 里显式声明
    `extra_imports`（能力保留，但安装时对用户可见）。
  - 新增绝对禁止层，声明任何权限都不解锁：`os` / `sys` / `subprocess` / `importlib` /
    `ctypes` / `pickle` / `inspect` / `builtins` 等。
  - 进程相关名字改为**取值即拦截**，堵住别名绕过；并拦掉
    `getattr` / `setattr` / `globals` / `locals` / `vars` 等自省原语。
  - **文档不再把它称作"沙箱"**：Python 运行时可自省、可变，静态分析无法穷尽绕过手法。
    它是加载前的风险筛查，不是安全边界。**安装第三方插件本质上等同于在你电脑上运行任意
    代码，请只安装可信来源。** 真正的隔离需要进程/容器级方案，已列入后续计划。

### 修复

- **Python 3.10 上打字打断会崩**：`asyncio.Task.cancelling()` 是 3.11+ 才有的 API，
  而本项目声明支持 `>=3.10`。该调用位于 `except CancelledError` 的处理体内，
  抛出的 `AttributeError` 按语义无法被同级 `except Exception` 接住，必然逃逸；
  它同时是下面那条桥停摆的引信。
- **跨端消息转达会永久静默失效**：桌面与 QQ 两个投递消费循环缺少异常保护——
  桌面侧的兜底块内部调用会二次抛出并冲出 `except`，QQ 侧整个循环体没有 `try`。
  任一抛出即杀死协程，此后 QQ↔桌面的转达全部石沉大海直到重启，**而日志里一个字都没有**。
  现在结算逐条保护、循环体整体兜底，并在任务异常退出时留下 ERROR。
- **任务账本故障会让白彻底不说话**：任务账本是纯观测性的旁路组件，但它的创建入口没有兜底
  （同模块其它方法早就有）。数据库被占用/损坏/磁盘满时，桌面端每条消息只收到红字
  "处理失败"，QQ 端则是纯粹的已读不回。现在降级为无账本运行、照常回复。
- **账本数据库坏了整个后端起不来**：建库排在读取配置之前且无保护，失败时抛裸 traceback，
  桌宠、QQ、B站、QQ空间全部无法启动。现在先读配置、建库失败降级并给出可操作提示。
- **群里说"别说话"没有反应**：活动租约的键分属两套命名空间——写入方用 `qq:group:{群号}`，
  而停止指令的读取/关闭方用了聊天上下文存储的 `group:{群号}`。两套键永不相交，
  于是活动窗口内未 @ 白的用户说"别说了"被完全忽略；即使 @ 白说停止，
  关闭动作也写在一把不存在的租约上，真实租约仍然有效、白继续接话。
  现在租约键收敛为唯一权威来源，两侧共用。
- **持续监听一次断线后永久卡死**：在途识别请求只在收到终态帧或发送失败时才清理。
  连接在"已排队"之后、终态帧之前断开时（识别较慢时这个窗口很宽），请求 ID 永久滞留，
  此后识别出的文字只堆进缓冲永不发送，麦克风永久停在"正在识别"，只能重启客户端。
- **持续监听的噪声会吞掉白正在说的整段话**：此前**任何**语音帧都会立即取消进行中的回复，
  而持续监听的分段是按音量/时长自动切出来的——一声咳嗽、关门、电视声都算一帧，
  且被取消的那轮不会重投递。现在把打断决策推迟到确认有语音内容之后：
  长按说话仍然立即打断（手动按下，意图明确），持续监听则等识别出文字再打断。
- **QQ 端在工具整理失败时已读不回**：工具跑完但主模型连续两次未能把结果整理成自然语言时，
  白在 QQ 上一个字都不说；而同样的失败在桌面端会推一张错误卡——这个不对称才是缺陷。
  现在会给一句简短告知，并带 60 秒会话级冷却以防供应商整体故障时刷屏。
- **每日记忆整理的四步从未执行过**：整理服务调用的清理入口名与存储层实际提供的不一致，
  第一步就抛 `AttributeError` 并被外层 `except` 接住，导致过期清理、长期记忆去重、
  核心记忆去重、以及遗忘曲线维护全部从未运行，日志里只有一行"整理失败"。
- **"搜索记忆"工具查不到核心记忆**：核心记忆条目是 dataclass，代码却按字典取值，
  异常被裸 `except` 吞掉，该分支恒返回空。现在取值方式修正，并且不再静默——
  同类错误正是靠裸 `except` 才藏了这么久。
- **面板示例里的真实 QQ 群号**：解除屏蔽群号输入框的示例值换成占位号。
- **安装器测试在非中文区域设置下必然失败**：`安装.bat` 是中文文件名，
  `cmd.exe` 解析命令行时使用系统 ANSI 代码页，无法表示这些汉字时会降级成 `??.bat`。
  与安装器本身无关，改为在这种情况下使用 8.3 短路径，系统禁用短名时明确跳过并说明原因，
  不再长期红着掩盖安装器的真实回归。

### 新增

- **ASR 服务地址可配置**：新增 `asr.base_url`。此前 ASR 地址写死为 SiliconFlow 云端，
  自建语音识别服务或需要走代理的用户无法指向自己的服务。留默认值行为不变。
- **端到端测试设施**（`tests/e2e/`）：可以真正把后端连同一套本地假上游（LLM/ASR/TTS）
  和假 NapCat 服务端一起拉起来，用真实 WebSocket 驱动。此前项目只有直接调函数的单测，
  像"回复流到一半插入语音帧会怎样"、"停止指令有没有真的让白闭嘴"这类**时序**问题
  测不出来。默认套件不跑它（`-m e2e` 显式开启），不影响日常测试速度。

### 可诊断性

- **QQ 消息因去重被丢弃时不再静默**：去重本身是必要的（重连补发会把同一条消息再送
  一遍），但此前丢弃时日志里一个字都没有。而 OneBot 的 `message_id` 是会话级整数，
  NapCat 重启后可能从较小值重新开始，该记录表保留 7 天——一旦新消息的 id 撞上历史
  记录，它会被当成"已处理"直接丢掉，用户看到白已读不回却查不到任何线索。
  现在补发重放撞重复记 DEBUG（属预期），**实时消息**被判重复记 WARNING 并提示可能
  的原因。去重判定逻辑未作任何改动。

### 验证

- Windows Python 单元测试：`982 passed, 19 skipped`（端到端另计）。
- 端到端测试：`8 passed`（真启动后端 + 假上游 + 假 NapCat，约 2 分 40 秒）。
- Electron 前端测试：`13 passed`。
- 新增 7 个专项测试文件，覆盖上述每一条修复。其中多条是**跨模块一致性断言**
  ——这批缺陷的共同特点是"两边各自内部自洽"，只测单边永远发现不了。
- 沙箱修复经实测对抗：12 类攻击写法（含别名绕过、私有模块转发、自省原语）全部拦截，
  6 类正常插件写法零误杀，内置 greeting 插件经真实插件管理器加载成功。
- 语音打断与停止指令均做了真实行为验证，而非仅测试通过；语音打断的守卫测试
  经"改回缺陷版本可复现失败"实测确认非装饰性断言。
- 一处历史用例（QQ 停止指令）此前用字面量构造了现实中不存在的租约，
  恰好与错误读法自洽而长期显示通过；已改为按生产路径构造。**断言本身未放宽。**

---

## [0.1.11] - 2026-07-14 · 安装、启动与服务器健康检查加固

这个修订版集中修复公开仓库在全新 Windows、Linux 服务器和 Docker 环境中的安装与启动问题。它只调整公开版的可移植流程，不带入作者本地工作区的固定路径或服务配置。

### 修复与改进
- **Windows 安装结果可信**：完整桌宠安装现在明确要求 Node.js LTS；前端依赖失败会停止并报错，不再把只有后端的半成品显示为安装成功；只装后端可显式使用 `安装.bat /backend-only`。
- **安全的一键启动**：`Start.bat` 从 `conf.yaml` 读取后端端口，以真实 `/health` 响应判断服务是否可用；已有健康服务会复用，端口被其它程序占用时会停止说明，不再强制结束进程或清理端口。
- **本地 TTS 启动更稳**：GPT-SoVITS 路径继续从环境变量或配置解析，等待实际 9880 监听成功；启动失败会降级到云端或文字，不会阻塞桌面聊天。
- **服务器向导不再假成功**：首次安装必须提供 API key，已有有效配置可保留旧 key；非本机监听会生成管理令牌；安装 systemd 后会等待 White Salary 的真实健康响应，失败时给出日志排查入口。
- **Docker 可直接访问后端**：容器显式监听 `0.0.0.0:12400` 并带健康检查；Compose 移除未被应用使用的独立 Chroma 服务，长期向量数据继续写入挂载的 `data` 目录。
- **控制面板错误可操作**：ComfyUI 已在线时可在任意系统复用；Linux / 服务器离线时明确提示需单独启动，Windows 缺路径、启动失败和超时分别返回真实原因，不再统一误报为 90 秒超时。
- **三条安装路线重新说明**：README、快速上手和安装指南分别说明 Windows 桌宠、Linux 服务器与 Docker 后端，明确虚拟环境、Node、管理令牌和可选本地工具边界。

### 验证
- Windows Python 全量测试：`930 passed, 19 skipped`。
- WSL / Linux 安装器行为测试：`18 passed`；`install.sh` / `server-setup.sh` 语法检查通过。
- 安装与设置定向回归：Windows `45 passed, 16 skipped`；Electron 前端测试 `7 passed`，`settings.js` 语法检查通过。
- `安装.bat /check` 实机通过，确认只检查 Python、Node 和虚拟环境能力，不执行安装。
- 本版本在从 `origin/main` 派生的独立工作树中完成；发布前继续执行固定路径、凭证和敏感信息扫描。
