...
它会逐项检查并用中文友好提示：
  ...
  4. 关键依赖是否装了（import 探测 fastapi / pydantic / ddgs / yt_dlp / PIL / mss 等）
  5. 前端依赖是否装了（frontend/node_modules）
  6. data/ 目录结构是否就绪
最后给出"下一步做什么"的建议。

用法：
    .venv\Scripts\python.exe scripts\first_run_check.py    # Windows
    .venv/bin/python scripts/first_run_check.py             # Linux / macOS
...
def venv_python_hint() -> str:
    """返回项目虚拟环境里的 Python 命令提示，避免引导用户装到全局环境。"""
    if sys.platform.startswith("win"):
        return r".venv\Scripts\python.exe"
    return ".venv/bin/python"


def pip_install_hint(extra: str = "") -> str:
    """返回在项目 .venv 中安装依赖的命令提示。"""
    suffix = f'"{extra}"' if extra else "."
    return f"{venv_python_hint()} -m pip install -e {suffix}"
...
def check_dependencies() -> bool:
    ...
    required = [
        ...
        ("httpx", "异步 HTTP 客户端（向量搜索/服务调用）", True),
        ("numpy", "数值计算", True),
    ...
    if missing_required:
        print("\n       缺少必需依赖。推荐先重新运行项目安装器：")
        if sys.platform.startswith("win"):
            print(f"       {C.BOLD}安装.bat{C.RESET}")
        else:
            print(f"       {C.BOLD}./install.sh{C.RESET}")
        print(f"       手动修复请在项目根目录执行：{C.BOLD}{pip_install_hint()}{C.RESET}")
    return all_ok
...
def main() -> int:
    ...
    if hard_ok:
        ...
        if not py_ok:
            print("       1) 安装 Python 3.10-3.12（推荐 3.11 或 3.12）。")
        if not dep_ok:
            if sys.platform.startswith("win"):
                print("       2) 重新双击 安装.bat，或手动安装后端依赖：")
            else:
                print("       2) 重新执行 ./install.sh，或手动安装后端依赖：")
            print(f"          {pip_install_hint()}")
        if not conf_ok:
            print("       3) 复制并填好 conf.yaml（至少一把主 LLM 密钥），见 docs/CONFIG.md。")
        print(f"\n       改完再跑一次本脚本确认：{C.BOLD}{venv_python_hint()} scripts/first_run_check.py{C.RESET}")
        exit_code = 1
