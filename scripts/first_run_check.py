...
def check_python() -> bool:
    ...
    说明：pyproject.toml 声明 requires-python >=3.11，但项目实测在 3.10 上
    也能正常运行（全量测试基线在 3.10 环境下通过）。因此这里以 3.10 为硬性
    下限：低于 3.10 直接判失败；3.10 通过但提示 pyproject 名义要求 3.11；
    3.11+ 完全达标。
    """
    title("1. Python 版本")
    ...
    if (major, minor) >= (3, 11):
        ok(f"Python {version_str}（满足 pyproject 声明的 >=3.11）。")
        return True
    if (major, minor) == (3, 10):
        ok(f"Python {version_str}（可运行；注意 pyproject.toml 名义要求 3.11，"
           f"实测 3.10 亦可，建议有条件时升级到 3.11+）。")
        return True
...
def check_dependencies() -> bool:
    ...
    optional = [
        ...
        ("openai", "OpenAI SDK（llm-openai，可选）"),
    ]
