...
|--------|------------|-------------|--------------------------|--------------|
| **硅基流动 SiliconFlow**（⭐ 新手首选，一家全包） | [cloud.siliconflow.cn](https://cloud.siliconflow.cn)（手机号注册） | 控制台 → API 密钥：[cloud.siliconflow.cn/account/ak](https://cloud.siliconflow.cn/account/ak)（以官网为准） | 主对话 `llm`（`provider: siliconflow`），同一把 key 可复制到 `llm_memory` / `llm_emotion` / `llm_vision` 等各节；`asr` / `tts` 兜底会自动复用 | 新号常送体验额度，另有若干小模型长期免费；以官网为准 |
...
| **看图 / 看屏幕**（视觉理解） | `llm_vision` | 硅基流动（GLM-4.1V / Qwen-VL 系列） | 留空则看图功能关闭 |
...
**填法 A — 直接改 `conf.yaml`（推荐先跑起来时用）**

先从模板复制一份自己的配置（只改 `conf.yaml`，**不要动 `conf.default.yaml` 模板**）：
...
启动后按 `Ctrl+,` 打开控制面板，进入 **LLM 设置**页，把各通道的 `api_key` / `model` / `base_url` 填进去保存即可。面板会写回 `conf.yaml`。
...
其它 7 个分角色模型（`llm_tool` / `llm_memory` / `llm_emotion` / `llm_vision` / `llm_postprocess` / `llm_detect` / `llm_background`）此时**留空**——对应的高级功能（工具判断、记忆提取、情感联动、看图……）会**降级或关闭**，但**完全不影响主聊天**。

想要更多功能时，再**逐个把对应节填上 key**即可。用硅基流动的话，通常把同一把 key 填到多个节就行（模型名按上表选）。

> 升级路线建议：主聊天 → 加 `llm_memory`（让白记住你）→ 加 `llm_vision`（能看图）→ 加语音（TTS/ASR）→ 加生图 / 生视频。
...
llm_vision:
  provider: "siliconflow"
  api_key: "sk-你的key"
  model: "THUDM/GLM-4.1V-9B-Thinking"     # 看图模型；也可换硅基流动的 Qwen-VL 系列
  base_url: "https://api.siliconflow.cn/v1"
```
...
**Q：key 填了怎么不生效？**
A：`conf.yaml` 在后端**启动时读入并缓存**，改完 key **必须重启后端**才生效（控制面板右下也有"重启后端"按钮）。改完先点【测试连通】确认，再重启。
...
A：语音要 `tts` / `asr` 配好（见第 6 节），看图要 `llm_vision` 填好。逐个对照第 2 节的表，缺哪个补哪个，然后【测试连通】+ 重启。
