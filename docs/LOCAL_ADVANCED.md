...
- 解压到任意目录（例如 `D:\ComfyUI_windows_portable`）。整合包自带启动脚本 `run_nvidia_gpu.bat`（N 卡）。
...
2. **启动脚本路径**：白在需要时可以**自动拉起** ComfyUI，靠的是启动脚本 `run_nvidia_gpu.bat` 的路径。默认写死为作者机器的路径，**你需要改成自己的**。

改法（**推荐用环境变量，避免改源码**）：

```bat
:: 指向你自己的 ComfyUI 启动脚本
set WS_COMFYUI_BAT=D:\ComfyUI_windows_portable\run_nvidia_gpu.bat
:: 图生视频 / 改图时，白会把输入图片复制进 ComfyUI 的 input 目录
set WS_COMFYUI_INPUT=D:\ComfyUI_windows_portable\ComfyUI\input
```
...
- 解压安装到任意目录（作者机器示例：`D:\AI_Tools\GPT-SoVITS`）。
...
> 使用前请把 `scripts/train_voice.bat` 里的 `cd /d "D:\AI_Tools\GPT-SoVITS"` 改成**你自己的 GPT-SoVITS 安装路径**（该脚本还引用了 GPT-SoVITS 的 `venv_new` 虚拟环境）。准备好一段白的目标音色素材，跑一次即可得到专属声音。
...
| GPT-SoVITS 目录 | 改 `scripts/train_voice.bat` 里的路径；推理走固定端口 `127.0.0.1:9880` |
| ffmpeg | 加系统 `PATH`，或放到项目探测的固定路径（见[第 3 节](#3-ffmpeg--语音--视频格式处理很多本地功能的前置)） |
...
```yaml
external_tools:
  comfyui_bat: "D:/ComfyUI_windows_portable/run_nvidia_gpu.bat"   # ComfyUI 启动脚本
  gpt_sovits_dir: "D:/AI_Tools/GPT-SoVITS"                        # GPT-SoVITS 安装目录
  ffmpeg_path: "D:/ffmpeg/bin/ffmpeg.exe"                          # ffmpeg 可执行文件
```

> **怎么判断你的版本走哪套**：打开 `conf.default.yaml` 搜 `external_tools`——**搜得到**就按方式二在 `conf.yaml` 填这几个路径；**搜不到**就用方式一的环境变量 / 固定路径（当前仓库即为此情况）。两套只是"同一件事的不同写法"，配一处即可，**不装任何本地组件时全部可以留空，白会自动用云端**。
