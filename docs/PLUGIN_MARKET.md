...
- 插件在加载前会走静态风险筛查和安全执行器；坏插件超时或异常时，不应该拖垮主聊天链路。
- ⚠️ **风险筛查不等于安全沙箱**。Python 运行时可自省、可变，静态分析无法穷尽绕过手法，
  它只能挡住常见的危险写法和低成本的恶意代码。**安装第三方插件本质上等同于在你的电脑上
  运行任意代码，请只安装你信任的来源。**
- 插件需要网络、文件、数据库、线程或第三方依赖时，必须在 `config.json` 的 `permissions`
  里显式声明（`network` / `filesystem` / `database` / `threads` / `extra_imports`），
  安装时这些声明对用户可见——看到不该要的权限就别装。
